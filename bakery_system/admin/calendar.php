...
$sql = "SELECT order_id, customer_id, order_type, status, pickup_date, delivery_date, total 
        FROM orders 
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .calendar-header {
            ...
            background: #8B4513; color: white; text-align: center; padding: 15px;
        }
        .calendar-days {
            display: grid; grid-template-columns: repeat(7, 1fr);
        }
        ...
        .day-number {
            font-weight: bold; color: #5D4037; margin-bottom: 5px;
        }
        .event {
            font-size: 11px; padding: 3px 6px; border-radius: 4px;
            margin-bottom: 3px; background: #e8f5e9; color: #2e7d32;
        }
        .event-pickup { background: #e3f2fd; color: #1565c0; }
        .event-delivery { background: #fff3e0; color: #e65100; }
...
<body>
    ...
    <div class="main">
        ...
        <div class="calendar-nav">
            ...
            <h2 style="color: #5D4037;"><?php echo $month_name; ?></h2>
                ...
                for ($day = 1; $day <= $days_in_month; $day++) {
                    ...
                    if (isset($events[$date])) {
                        foreach ($events[$date] as $event) {
                            $type_class = $event['order_type'] == 'pickup' ? 'event-pickup' : 'event-delivery';
                            echo '<div class="event ' . $type_class . '">';
                            echo '#' . $event['order_id'] . ' - ' . ucfirst($event['order_type']);
                            echo '</div>';
                        }
                    }
